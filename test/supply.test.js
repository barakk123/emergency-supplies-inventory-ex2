...
describe('Supplies API Endpoints', () => {
    ...
    describe('PUT /supplies/:supplyName', () => {
        ...
        it('returns 409 if conflict on supplyName', async () => {
            ...
            emergencySupplier.getSupplyByName.mockResolvedValueOnce(supplyToUpdate);
        
            emergencySupplier.getSupplyByName.mockResolvedValueOnce(existingSupply);
        
            const updateData = { supply_name: 'ExistingWater' }; 
            const response = await request(app).put(`/supplies/${supplyToUpdate.supply_name}`).send(updateData);
        
            expect(response.statusCode).toBe(409);
            expect(response.text).toContain('Supply with the updated name already exists.');
        });

        it('returns 404 if supply not found', async () => {
            const nonExistingSupplyName = 'NonExistingWater';
            const updateData = { supply_name: 'NonExistingWaterUpdated', category: 'Core', unit_price: 2.80, supplier: "Sami", location: "Israel", quantity: 100 };
        
            emergencySupplier.getSupplyByName.mockResolvedValueOnce(null); 
            emergencySupplier.updateSupply.mockResolvedValueOnce(null); 
        ...
        it('returns 500 when an error occurs', async () => {
            const supplyToUpdate = { supply_name: 'ExistingWater', category: 'Core', unit_price: 2.80, supplier: "Sami", location: "Israel", quantity: 100 };
            const updatedData = { category: 'Medical', unit_price: 3.50, supplier: "Danny", location: "USA", quantity: 50 };
        
            emergencySupplier.updateSupply.mockRejectedValue(new ServerError());
        
            const response = await request(app).put(`/supplies/${supplyToUpdate.supply_name}`).send(updatedData);
